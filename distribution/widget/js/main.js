function initRenderDist(arUsers) {
  ...
  for (var pipeline in arPipelines) {
    ...
    for (var userID in arUsers) {
      ...
      arBalances[pipeline][userID] = currentUser.rate - 0;


      currentPipeline.append('<div class="item-range user-' + pipeline + '-' + userID + '"><b>' + currentUser.name + '</b><div style="width: 150px;">\
          <input data-pipeline="'+pipeline+'" data-user="'+userID+'" id="user-' + pipeline + '-' + userID + '" class="js-range user-' + pipeline + '-' + userID + '"/>\
      ...
      var $range = elRange[pipeline + '-' + userID] = $(document.querySelector(".js-range.user-" + pipeline + '-' + userID)).ionRangeSlider({
        ...
        onChange: function(data) {
          var object = $(data.input[0]);
          var objPipeline = object.attr("data-pipeline");
          var objUser = object.attr("data-user");
          var slideCurrent = elRange[objPipeline + '-' + objUser];
          arBalances[objPipeline][objUser] = data.from;
          var currentBalance = 0;
          for (var keyUser in arBalances[objPipeline]) {
            currentBalance += arBalances[objPipeline][keyUser];
          }
          if (currentBalance > 100) {
            var proc = (currentBalance - 100);
            slideCurrent.update({"from": (arBalances[objPipeline][objUser] - proc)});
          }
        },
